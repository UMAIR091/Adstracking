...
export async function getValidAccessToken(
  ...
  if (!ds.access_token) throw new Error("This connection has no access token. Reconnect Google.");
  ...
  if (!ds.refresh_token) throw new Error("Token expired and no refresh token. Reconnect Google.");

  const refreshed = await refreshAccessToken(decrypt(ds.refresh_token));
