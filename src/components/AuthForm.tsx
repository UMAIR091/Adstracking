...
export function AuthForm({ mode, next = "/dashboard" }: { mode: "login" | "signup"; next?: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (isSignup) {
      // Real-time UI shows the rules; guard here so a weak password can't slip
      // through (also covers browsers/autofill that bypass field validation).
  ...
  return (
    <div className="mx-auto w-full max-w-sm">
      ...
      <div className="rounded-2xl border border-ink-200 bg-surface p-7 shadow-sm">
        ...
        <button
          onClick={google}
          className="mb-4 flex w-full items-center justify-center gap-2 rounded-lg border border-ink-300 px-4 py-2.5 text-sm font-medium text-ink-700 hover:bg-ink-50"
        >
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          {info && <div className="rounded-lg bg-success-50 px-3 py-2 text-sm text-success-700">{info}</div>}
          <div>
            <label className="mb-1 block text-sm font-medium text-ink-700">Email</label>
            <input type="email" className={inputClass} value={email} onChange={(e) => setEmail(e.target.value)} required />
          </div>
          {isSignup ? (
            <PasswordField value={password} onChange={setPassword} showMeter autoComplete="new-password" />
          ) : (
            <div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full rounded-lg bg-brand-solid px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-solid-hover disabled:opacity-50"
          >
